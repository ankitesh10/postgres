...
TEXT has no bounds on number of characters, as recipe can be huge
```sql
INSERT INTO recipes
  (title, body)
VALUES
...
Images of shakshuka
```sql
SELECT * FROM recipes_photos WHERE recipe_id=4; -- Problem is code will not know the recipe id
```
...
Photo short name for row in recipes_photos

INNER JOIN - one to one matching, it gets common data
